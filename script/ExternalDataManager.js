export class ExternalDataManager {
	static async addGpxDataSource(viewer, dataSourceInfo, markerFilePath) {
		...
		for (const entity of dataSource.entities.values) {
			...
			if (Cesium.defined(entity.billboard)) {
				ExternalDataManager.#setBillboardProperties(entity.billboard, markerFilePath);
			}

			if (Cesium.defined(entity.label)) {
				ExternalDataManager.#setLabelProperties(entity.label);
			}
	...
	static updateDataSourceVisibility(viewer, entitiesId, visible) {
		const dataSource = ExternalDataManager.getDataSource(viewer, entitiesId);
	...
	static getEntitiesTree(viewer, entitiesCollectionId) {
		const dataSource = ExternalDataManager.getDataSource(entitiesCollectionId);
		const entities = [];
